...
class LongTermAggregatedStatisticsHelperTest(unittest.TestCase):
    ...
    def test_truncates_globally_and_keeps_entity_with_empty_values(self):
        ...
        self.assertEqual(500, result["data"]["truncation"]["count_returned"])
        self.assertEqual(501, result["data"]["truncation"]["count_total_before_truncation"])
        self.assertEqual(500, result["data"]["truncation"]["limit"])
        self.assertEqual(500, result["data"]["truncation"]["by_entity_id"]["sensor.one"]["count_returned"])
        self.assertTrue(result["data"]["truncation"]["by_entity_id"]["sensor.two"]["hidden_by_global_limit"])
